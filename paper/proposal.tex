...
multiple times. We refer to the number of times each page is sampled by $T$. As
...
it, and then manually list out some functions that are heavily involved in
...
All experiments were run with low system load. The experiments are run all
...
Using a training set created on System 1\footnote{poppler/0003} we were able to
...
volume is normal. If not, it guesses that the volume is hidden. We call the
number of trials a run of the experiment $S$.
