...
attacker could perform the training phase on a different machine with similar
hardware, but we did not try it.}. Next, the actual attack happens: the attacker
...
% TODO: The above says "input file", and it *is* the input file (to a component
% of a toolchain) in our implementation, but in the way we're describing it,
% "input file" means the original unsorted list.
...
experiment\endnote{links/0014} on System 1 to recover victim samples from
another experiment\endnote{links/0015} on System 2. The result was that 76\% of
the pages were recovered. Using the training set from System 2 and the victim
samples from System 2, the recovery was successful 83.7\% of the time. So it is
better to run the training on the victim's machine, but the attack still works
by training on a different machine.
...
and $S=1$\endnote{poppler/0006}, the correct PDF was identified $124$ times out
...
decide right $93$ times out of $100$. In a longer run\endnote{truecrypt/0006}
